...
版本必須使用完整的 `<major>.<minor>.<patch>` 格式，例如 `0.3.0`：
...
語意化版本 tag 會觸發 GitLab pipeline。Pipeline 使用 `git-cliff` 產生同一份
release notes，建立 GitLab Release，並把 tag 與 release notes 發布到
`cwa-tw/claude_code_docker` GitHub repository。GitLab CI/CD 必須提供 masked、
protected 的 `CWA_GITHUB_TOKEN`，權限需涵蓋該 GitHub repository 的
`Contents: Read and write`；GitLab 也必須保護 release tag pattern。
